...
import { DirectionalData, ITelemetryData } from "shared";
...
const TAG = "TrackMapViewModel";
export function useTrackMapViewModel() {
  ...
  useEffect(() => {
    const packetListener = network.onPacket((packet) => {
      setForza(packet);
    });
  ...
  useEffect(() => {
    const replayListener = replay.onPacketEvent((packet) => {
      setForza(packet);
    });
