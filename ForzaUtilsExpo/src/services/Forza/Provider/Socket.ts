...
import { Udp_rinfo } from "../Network.types";
...
export class Socket implements ISocket {
    ...
    return new Promise((resolve, reject) => {
      ...
      const bindListeningHandler = () => {
        if (this.udpSocket) {
          ...
          Logger.log(this.tag, `Socket is undefined at listening event`);
  ...
  private errorHandler(ev: Error | any) {
    ...
    try {
      ...
      Logger.error(this.tag, `Socket error: ${err?.message}`);
    }
  }
  private closeHandler(ev: Error | any) {
    this.eventEmitter.emit(SocketEventType.CLOSE, ev);
    this.udpSocket = undefined;
    Logger.error(this.tag, `Socket closed: ${ev?.message}`);
  }
